...
1. Should changing status require a reason? (e.g., if marking "lost", must provide `lost_reason`)
   1. _Optional field - select with a dropbox for common reasons with an option for “Other”_
2. When you mark an inquiry as "confirmed", should it auto-create a booking, or is that a separate action?
   1. _Yes, auto-create booking. Or prompt user to create booking_
...
3. Does the build order match your priorities? _Yes_
